...
async function AboutSection({ params: { lng } }) {
	...
	return (
		<section id='about' className='text-sky-900 bg-gray-100 py-16 h-screen'>
			<div className='container mx-auto flex items-center h-full'>
				...
				/>
				<div>
					<h2 className='text-sky-900 text-3xl lg:text-4xl font-bold mb-8'>
						{t("about-us")}
					</h2>
					{descriptions.map((description, index) => (
						<p
							key={index}
							className={`text-sky-800 text-base md:text-lg ${
								index >= 1 && index <= 3
									? "text-justify mb-6"
									: "mb-4"
							}`}
